function [X_pos X_neg k] = affineTrainG(dataPath, sz, opt, param, num_p, num_n, forMat, p0,begin,frame)
...
% p0: aspect ratio in the first frame  第一帧图像的高宽比
...
if size(img_color,3)==3
    ...
    img_color=double(img_color)/255;
end
 img= frame/255;
%%----------------- positive templates----------------%%
...
param.param0 = repmat(affparam2geom(param.est(:)), [1,n]);  %扩展成n*6的矩阵
randMatrix = randn(6,n);
sigma = [1, 1, .000, .000, .000, .000];
param.param = param.param0 + randMatrix.*repmat(sigma(:),[1,n]); %相当于在原图像块周边采样

k = affparam2mat(param.param); % Extract or Warp Samples which are related to above affine parameters
wimgs = warpimg(img, k, sz);

m = prod(opt.tmplsize);  % 相当于m=32*32
X_pos = zeros(m, n);
for i = 1: n
    X_pos(:,i) = reshape(wimgs(:,:,i), m, 1);  %将采样得到的positive templates按列向量存储
end
...
param.param0 = zeros(6,n);      % Affine Parameter Sampling
param.param = zeros(6,n);
param.param0 = repmat(affparam2geom(param.est(:)), [1,n]); %para,.param0=6*n
randMatrix = randn(6,n);
