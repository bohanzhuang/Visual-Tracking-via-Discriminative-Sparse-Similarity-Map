...
% opt: initial parameters  opt = struct('numsample',100, 'affsig',[4, 4, .02, .03, .001,.000]);
% patchsize: the size of each patch   [6,6]
% patchnum: the number of patches in one candidate  
% Fisize: the number of cluster centers   50
...
image = warpimg(img, param0, opt.tmplsize); % 得到目标图像块

patch = zeros(prod(patchsize), prod(patchnum));  %初始化patch,用来存储用过sliding window采集到的图像块
% 一共采集 patchnum(1)*patchnum(2)个图像块
...
for j = 1: patchnum(1)                   % sliding window
    for k = 1:patchnum(2)
        ...
        patch(:, l) = reshape(data,numel(data),1);  %numel(data)返回data的元素个数
        l = l+1;
    end
end
